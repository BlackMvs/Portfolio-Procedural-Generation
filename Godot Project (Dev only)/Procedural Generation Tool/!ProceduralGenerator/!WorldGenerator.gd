...
signal generation_progress(current: int, total: int, status: String)
...
## Select this if you have a console that will display messages to it such as [method ConsoleManager.log()]
@export var displayConsole : bool = true
## Only used during the development of the project! Will give error if dev file are missing! [br]
## Select this if we have a loading screen and want to display this information to the screen
...
func _ready() -> void:
	...
	ConsoleManager.log_important("Scene nodes loaded in: " + str((nodesLoadedTime - startTime)) + " ms")
	...
	#Check if using debug setting 
	if hardCodedSeed:
		rng.seed = Stats.currentSeed
		# Initialize noise generator
		noise = Stats.fastNoiseLiteGenerator
	else: 
	...
	if voxel_types.is_empty():
		...
		ConsoleManager.log_warning("Warning: No voxel types configured!")
		return
	...
	ConsoleManager.log_important("Ready function took: " + str((readyEndTime - readyStartTime)) + " ms")
	ConsoleManager.log_important("Total load time was: " + str((readyEndTime - startTime)) + " ms")
	...
	ConsoleManager.log("=== World Generation Performance ===")
	ConsoleManager.log("World Size - X:" + str(world_size_x) + " Z:" + str(world_size_z) + " Y:"  + str(world_size_y))
	ConsoleManager.log("World generation complete! Time: " + str(end_time - start_time) + "ms")
	ConsoleManager.log("Total voxels spawned: " + str(get_child_count()))
	#TODO fix this 
	ConsoleManager.log("Max possible cubes: " + str(total_cubes))
	ConsoleManager.log("Cubes per second: " + str(total_cubes / (generation_time / 1000.0)))
	ConsoleManager.log("====================================")
...
func setup_voxel_types():
	...
	for config in voxel_configurations:
		...
		if not config.voxel_scene:
			ConsoleManager.log_warning("Voxel '" + config.voxel_name + "' has no scene - skipping")
			continue
		...
		if voxel.mesh_node_paths.is_empty():
			ConsoleManager.log_warning("Voxel '" + config.voxel_name + "' has no MeshInstance3D nodes")
		...
		if config.layer_names.is_empty():
			...
			for layer_name in config.layer_names:
				if voxels_by_layer.has(layer_name):
					...
					ConsoleManager.log_warning("Voxel '" + config.voxel_name + "' references unknown layer '" + layer_name + "'")
	...
	ConsoleManager.log("=== Terrain Setup ===")
	for layer in sorted_layers:
		if layer:
			ConsoleManager.log("  " + layer.layer_name + ": " + str(voxels_by_layer[layer.layer_name].size()) + " voxels")
...
func _update_chunks_around_player(is_initial_load : bool = false) -> void:
	...
	# STEP 3: LOAD
	var total = chunks_to_load.size()
	var count = 0
	for entry in chunks_to_load:
		generate_chunk(entry.key.x, entry.key.z)
		count += 1
		
		# Emit loading screen progress only on the initial world load
		if is_initial_load and loadingScreen:
			generation_progress.emit(count, total, "Generating chunks...")
...
func _generate_fixed_world() -> void:
	var total_chunks = world_size_x * world_size_z
	...
	for x in range(0, world_size_x):
		for z in range(0, world_size_z):
			...
			chunks_generated += 1
			if loadingScreen:
				generation_progress.emit(chunks_generated, total_chunks, "Generating chunks...")
